...
check('матчер учитывает запрещённый старый фрагмент как игра', () => {
  const checkRule = { includes: 'status =', notIncludes: 'status = "new"' }
  assert.equal(passesCodeCheck('status = "new"', checkRule), false)
  assert.equal(passesCodeCheck('status = "paid"', checkRule), true)
})

check('матчер учитывает минимальное число повторов как игра', () => {
  const checkRule = { includes: 'print(', minOccurrences: 2 }
  assert.equal(passesCodeCheck('print(1)', checkRule), false)
  assert.equal(passesCodeCheck('print(1)\nprint(2)', checkRule), true)
})
...
check('голый метод в проверке узнаётся в вызове у объекта', () => {
  const mission = lab('B-002M', {
    starter: 'statuses = ["new"]\nstatuses.append("review")',
    checks: ['append("paid")'],
  })
  assert.ok(!requiredBy(mission, 'python').has('append()'))
})
...
check('замена старого значения не считается выполненной на старте', () => {
  const mission = {
    ...lab('D-005', { starter: 'status = "new"' }),
    task: {
      starterCode: 'status = "new"',
      codeChecks: [{ label: 'Статус заменён', includes: 'status =', notIncludes: 'status = "new"' }],
    },
  }
  assert.deepEqual(checksSatisfiedByStarter(mission, 'python').map(item => item.byCode), [false])
  assert.equal(of(run([mission]), VIOLATIONS.CHECK_PASSES_ON_STARTER).length, 0)
})

check('недостающие повторы не считаются выполненными на старте', () => {
  const mission = {
    ...lab('D-006', { starter: 'print(1)' }),
    task: {
      starterCode: 'print(1)',
      codeChecks: [{ label: 'Два вывода', includes: 'print(', minOccurrences: 2 }],
    },
  }
  assert.deepEqual(checksSatisfiedByStarter(mission, 'python').map(item => item.byCode), [false])
  assert.equal(of(run([mission]), VIOLATIONS.CHECK_PASSES_ON_STARTER).length, 0)
...
check('голый метод проверки совпадает с ранее показанным методом объекта', () => {
  const findings = run([
    lab('G-004', { starter: 'orders.groupby("status").sum()' }),
    lab('G-005', { starter: 'orders = table', checks: ['groupby("city")'] }),
  ])
  assert.ok(!of(findings, VIOLATIONS.REQUIRED_BEFORE_SHOWN).some(item => item.token === 'groupby()'),
    JSON.stringify(findings))
})

check('квалифицированный вызов библиотеки не смешивается с голым методом', () => {
  const findings = run([
    lab('G-006', { starter: 'series.isna()' }),
    lab('G-007', { starter: 'import pandas as pd', checks: ['pd.isna(value)'] }),
  ])
  assert.ok(of(findings, VIOLATIONS.REQUIRED_BEFORE_SHOWN).some(item => item.token === 'pd.isna()'),
    JSON.stringify(findings))
})

check('ступень метода сохраняется при проверке без имени объекта', () => {
  const findings = run([
    lab('G-008', { stage: 'modified', starter: 'orders.duplicated(subset=["id"], keep=False)' }),
    lab('G-009', { stage: 'independent', starter: 'orders = table', checks: ['duplicated(subset=["id"], keep=False)'] }),
  ])
  assert.ok(!of(findings, VIOLATIONS.LADDER_GAP).some(item => item.token === 'duplicated()'),
    JSON.stringify(findings))
})

check('метод остаётся тем же при смене имени объекта', () => {
  const findings = run([
    lab('G-009A', { stage: 'modified', starter: 'events.duplicated(subset=["id"], keep=False)' }),
    lab('G-009B', { stage: 'independent', starter: 'orders = table', checks: ['orders.duplicated(subset=["id"], keep=False)'] }),
  ])
  assert.ok(!of(findings, VIOLATIONS.LADDER_GAP).some(item => item.token === 'orders.duplicated()'),
    JSON.stringify(findings))
})

check('одноимённые функции разных библиотек не смешиваются', () => {
  const findings = run([
    lab('G-009C', { starter: 'np.array([1]).sum()' }),
    lab('G-009D', { starter: 'import pandas as pd', checks: ['pd.array([1])'] }),
  ])
  assert.ok(of(findings, VIOLATIONS.REQUIRED_BEFORE_SHOWN).some(item => item.token === 'pd.array()'),
    JSON.stringify(findings))
})

check('перенос тоже считается предыдущей практикой конструкции', () => {
  const findings = run([
    lab('G-010', { stage: 'transferred', starter: 'gap = abs(sample - population)' }),
    lab('G-011', { stage: 'independent', starter: 'sample = 0.7\npopulation = 0.5', checks: ['abs(sample - population)'] }),
  ])
  assert.equal(of(findings, VIOLATIONS.LADDER_GAP).length, 0, JSON.stringify(findings))
})
